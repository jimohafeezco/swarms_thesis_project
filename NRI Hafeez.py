...
plt.savefig('images/'+'msetrain_guppies'+'png',dpi=300)
# plt.ylabels("")
...
plt.savefig('images/'+'accuracytrain'+'png',dpi=300)
...
plt.savefig('images/'+'KL_divertrain'+'png',dpi=300)
...
plt.grid(color="whitesmoke")
plt.savefig('images/'+'maetraiguppies'+'png',dpi=300)

# plt.figure()
# plt.plot(mape_train_loses)
# plt.title("MAPE loses(train)")
# plt.xlabel("epochs")
# plt.savefig('images/'+'mapetrain'+'png',dpi=300)
...
plt.savefig('images/'+'maevalloss'+'png',dpi=300)

# plt.figure()
# plt.plot(mape_val_loses)
# plt.title("MAPE loses(validation)")
# plt.xlabel("epochs")
# plt.savefig('images/'+'mapeloss'+'png',dpi=300)
...
plt.savefig('images/'+'mseval'+'png',dpi=300)
...
plt.grid(color="whitesmoke")
plt.ylabel("reconstruction accuracy")
plt.savefig('images/'+'accuracyvalidation'+'png',dpi=300)
...
plt.xlabel("epochs")
plt.grid(color="whitesmoke")
plt.savefig('images/'+'kldiver_validation'+'png',dpi=300)
...
# mse_loss, predi, g_truth=test_model(test_loader, step=1)
...
#     plt.figure(figsize=(15, 15))
    for i in range(1,12):
        if i<=10:
            plt.subplot(4, 3, i)
            plt.plot(predicted_features[i-1,:,0],predicted_features[i-1,:,1],'b')
            plt.plot(target_features[i-1,:,0],target_features[i-1,:,1],  'r--')
            plt.title("agent "+str(i),fontsize = 8)
    plt.tight_layout()
    plt.savefig('images/'+'agents_traj'+str(step),dpi=300)
#     plt.show()
step=1
...
#     plt.figure(figsize=(15, 15))
    for i in range(1,12):
        if i<=10:
#             plt.subplot(4, 3, i)
            plt.plot(predicted_features[i-1,:500,0],predicted_features[i-1,:500,1])
            plt.plot(target_features[i-1,:500,0],target_features[i-1,:500,1])
            plt.title("agent "+str(i),fontsize = 8)
    plt.tight_layout()
#     plt.savefig('images/'+'agents_traj'+str(step),dpi=300)
#     plt.show()
...
#     time=[5, 100, 500, 1000, 1500]
    ...
    for t in range(0, 1100,100):
        ...
        for i in range(10):
            ...
            plt.savefig('newimages/'+'guppiest_'+str(t),dpi=300)
...
#     plt.figure(figsize=(15, 15))
    ...
    ax1.plot(predicted_features[1,:,0],predicted_features[1,:,1],'b',label='prediction')
    ax1.plot(target_features[1,:,0],target_features[1,:,1],  'r--',label='ground truth')
    ax1.legend()
    ...
    plt.savefig('images/'+'traj_error'+str(step)+'png',dpi=300)
step=20
mse_loss, mae_Loss, mape_loss, predi, g_truth=test_model(test_loader, step)
plot_result(predi, g_truth, step=step)
...
plt.ylabel("MAE")
plt.grid(color="whitesmoke")
plt.savefig('images/'+'mae_test_guppies',dpi=300)
...
plt.ylabel("MSE")
plt.grid(color="whitesmoke")
plt.savefig('images/'+'mse_test_guppies',dpi=300)
...
#     for t in range(10, 990,50):
    ...
    for i in range(10):
#             plt.plot(predicted_features[i,:,0],predicted_features[i,:,1])
            plt.plot(target_features[i,:,0],target_features[i,:,1])
    plt.xlabel("x position(m)")
    plt.ylabel("y position(m)")
...
np.save('images/'+'prediction_data' + '.npy', predicted_features)
np.save('images/'+'target_data' + '.npy', target_features)
...
line_ani.save('images/'+'pred_tracj.gif', writer='pillow')
...
line_ani.save('images/'+'target_tracj.gif', writer='pillow')
...
dataX1= predicted_features[:,:,0]
dataY1= predicted_features[:,:,1]
...
def animate_func(i):
    ...
    ax.scatter(dataX[:,i], dataY[:,i], c='blue', marker='.')
    ax.scatter(dataX1[:,i], dataY1[:,i], c='red', marker='.')
...
line_ani.save('images/'+'pred_target_tracj.gif', writer='pillow')
